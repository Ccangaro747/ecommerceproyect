...
const MenuList = ({ open, setOpen }) => {
  ...
  return (
    <div className={`${open ? 'visible opacity-100' : 'invisible opacity-0'} transition-all fixed inset-0 bg-black/50 flex justify-end items-start`}>
      <aside className={`transform ${!open ? '-translate-x-48' : 'translate-x-0'} transition-transform w-48 bg-white rounded-lg shadow-lg overflow-hidden mt-8`}>
        <div 
        ...
        <nav className="flex flex-col gap-5 px-4 text-gray-800">
          ...
          <Link className="hover:text-gray-500 text-lg" onClick={handleClose} href={"/"}>Inicio</Link>
          <Link className="hover:text-gray-500 text-lg" onClick={handleClose} href={"/productos/all"}>Productos</Link>
          ...
          <Link className="hover:text-gray-500 text-lg mb-4" onClick={handleClose} href={"/admin"}>Panel de administracion</Link>
